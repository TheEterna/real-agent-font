...
// 支持的输入模式
export type InputMode = 'geek' | 'multimodal' | 'command'
...
export function useModeSwitch() {
  ...
  const getModeFromRoute = (): InputMode => {
    ...
    if (modeParam && ['geek', 'multimodal', 'command'].includes(modeParam)) {
  ...
  const isCommandMode = computed(() => currentMode.value === 'command')
