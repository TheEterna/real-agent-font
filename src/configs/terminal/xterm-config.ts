...
export const DEFAULT_TERMINAL_CONFIG: TerminalConfig = {
  ...
  rightClickCopiesSelection: true,
...
export function validateTerminalConfig(config: Partial<TerminalConfig>): boolean {
  try {
    ...
    if (config.cols && (config.cols < 1 || config.cols > 1000)) {
      console.warn('Invalid cols value:', config.cols)
      return false
    }

    if (config.rows && (config.rows < 1 || config.rows > 100)) {
      console.warn('Invalid rows value:', config.rows)
      return false
    }
